...
namespace BetterAPI.Guidelines.Prefer
{
    internal sealed class PreferActionFilter : IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            {
                foreach (var _ in prefer.Where(token => token.Equals(Constants.Prefer.ReturnMinimal, StringComparison.OrdinalIgnoreCase)))
                {
                    context.HttpContext.Response.Headers.Add(ApiHeaderNames.PreferenceApplied, Constants.Prefer.ReturnMinimal);
                    minimal = true;
                }

                // this is the current default, but we still want to indicate the preference was followed
                foreach (var _ in prefer.Where(token => token.Equals(Constants.Prefer.ReturnRepresentation, StringComparison.OrdinalIgnoreCase)))
                {
                    context.HttpContext.Response.Headers.Add(ApiHeaderNames.PreferenceApplied, Constants.Prefer.ReturnRepresentation);
                    minimal = false;
                }
            }
            else
