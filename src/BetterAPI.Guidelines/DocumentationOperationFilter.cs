...
namespace BetterAPI.Guidelines
{
    ...
    internal sealed class DocumentationOperationFilter : IOperationFilter
    {
        ...
        private static string CreateOperationId(OperationFilterContext context)
        {
            ...
            foreach (var producesResponseType in context.MethodInfo.GetCustomAttributes(true).OfType<ProducesResponseTypeAttribute>())
            {
        ...
        {
            foreach (var response in operation.Responses.Where(response => response.Key == Constants.CreatedStatusString))
